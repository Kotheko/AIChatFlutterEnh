...
// Импорт пакета для управления окном (только для desktop)
...
// Импорт для определения платформы
import 'dart:io';
...
void main() async {
  try {
    ...
    // Инициализация window_manager только для desktop платформ (Windows, Mac, Linux)
    ...
    debugPrint('Settings: ${settingsService.getAllSettings()}');

    // Проверяем наличие сохраненных данных
    final dataService = DataService();
    final hasData = await dataService.hasSavedData();
    if (hasData) {
      debugPrint('Found saved data, will be loaded on app start');
    } else {
