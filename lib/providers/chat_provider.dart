...
// Импорт сервиса для работы с данными
...
class ChatProvider with ChangeNotifier {
  ...
  Future<void> _loadHistory() async {
    try {
      ...
      final dbMessages = await _db.getMessages();
      
      // Если в базе есть сообщения, используем их
      if (dbMessages.isNotEmpty) {
        _messages.clear();
        _messages.addAll(dbMessages);
        _log('Loaded ${_messages.length} messages from database');
        notifyListeners();
        return;
      }
      
      // Если база пуста, пробуем загрузить из JSON файла
      _log('Database is empty, checking for JSON backup...');
      final jsonData = await _dataService.loadAllData();
      
      if (jsonData != null && jsonData.containsKey('messages')) {
        final List<dynamic> jsonMessages = jsonData['messages'] as List<dynamic>;
        ...
        if (jsonMessages.isNotEmpty) {
          ...
          for (final msg in jsonMessages) {
            ...
            final message = ChatMessage(
              ...
              modelId: messageMap['modelId'] as String?,
              tokens: messageMap['tokens'] as int?,
              cost: messageMap['cost'] as double?,
            );
            _messages.add(message);
            
            // Также сохраняем в базу данных для будущего использования
            await _db.saveMessage(message);
          }
          
          _log('Loaded ${_messages.length} messages from JSON backup and saved to database');
          
          // Загружаем аналитику из JSON если есть
          if (jsonData.containsKey('statistics')) {
            _log('Statistics found in JSON backup');
            // Аналитика сессии не требует специальной обработки
          }
        } else {
          _log('No messages found in JSON backup');
        }
      } else {
        _log('No JSON backup found');
      }
  ...
  // Метод обновления данных после применения настроек
  Future<void> refreshData() async {
    try {
      _log('Refreshing data after settings update...');
      ...
      // Загружаем модели
      await _loadModels();
      _log('Models refreshed: ${_availableModels.length} models');
      
      // Загружаем баланс
      await _loadBalance();
      _log('Balance refreshed: $_balance');
